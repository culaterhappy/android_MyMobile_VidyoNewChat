...
#include <stdio.h>
#include <string.h>
#include "lib/include/VidyoClient.h"
...
#define  LOG_TAG    "VidyoSampleActivity.libplasma"
				   ...
				   VidyoVoidPtr data)
{
	...
	else if(event == VIDYO_CLIENT_OUT_EVENT_CALL_STATE ){
	    /*
	    VidyoClientOutEventCallState* eventState = (VidyoClientOutEventCallState *) param;


        char statedes[32];
        memset(statedes,0,sizeof(statedes)-1);
        switch(eventState->callState){
            case VIDYO_CLIENT_CALL_STATE_IDLE:
                sprintf(statedes,"VIDYO_CLIENT_CALL_STATE_IDLE");
                break;
            case VIDYO_CLIENT_CALL_STATE_RINGING:
                sprintf(statedes,"VIDYO_CLIENT_CALL_STATE_RINGING");
                break;
            case VIDYO_CLIENT_CALL_STATE_CALLING:
                sprintf(statedes,"VIDYO_CLIENT_CALL_STATE_CALLING");
                break;
            case VIDYO_CLIENT_CALL_STATE_JOINING:
                sprintf(statedes,"VIDYO_CLIENT_CALL_STATE_JOINING");
                break;
            case VIDYO_CLIENT_CALL_STATE_IN_CONFERENCE:
                sprintf(statedes,"VIDYO_CLIENT_CALL_STATE_IN_CONFERENCE");
                break;
            case VIDYO_CLIENT_CALL_STATE_DISCONNECTING:
                sprintf(statedes,"VIDYO_CLIENT_CALL_STATE_DISCONNECTING");
                break;
            default:
                break;
        }
	    LOGI("GuiOnOutEvent enter Event = %d...VIDYO_CLIENT_OUT_EVENT_CALL_STATE...callState->%s...\n",(int) event,statedes);
	    */
...
//doCallByNameDirect
JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_app_ApplicationJni_doCallByNameDirect(JNIEnv *env, jobject jobj,  jstring userName)
{

    const char *usernameC = (*env)->GetStringUTFChars(env, userName, NULL);
    LOGI("doCallByNameDirect...start...userName->%s...\n",usernameC);
	doCallByNameDirect(usernameC);
}

//void doJoinByName(const char *displayName, const char *pin);
JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_app_ApplicationJni_doJoinByName(JNIEnv *env, jobject jobj,  jstring userName,jstring pin)
{
    LOGI("doJoinByName...start...userName->%s...pin->%s...\n",userName,pin);
    const char *usernameC = (*env)->GetStringUTFChars(env, userName, NULL);
    const char *pinC  = (*env)->GetStringUTFChars(env, pin, NULL);
	doJoinByName(usernameC, pinC);

}


//VIDYO_CLIENT_REQUEST_SET_WINDOW_SHARES
JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_app_ApplicationJni_setWindowShares(JNIEnv *env, jobject jobj,  int currApp,jstring remoteAppUri)
{
    const char *remoteAppUri_ = (*env)->GetStringUTFChars(env, remoteAppUri, NULL);

    LOGI("setWindowShares...start...remoteAppUri->%s...\n",remoteAppUri_);

    VidyoClientWindowShares requestShare;
    memset(&requestShare, 0, sizeof(requestShare));

    VidyoClientRequestWindowShares rws;
    rws.requestType = LIST_SHARING_WINDOWS;
    rws.shareList = requestShare;

    VidyoUint ret = VidyoClientSendRequest(VIDYO_CLIENT_REQUEST_GET_WINDOW_SHARES,&rws, sizeof(VidyoClientRequestWindowShares));
    if (ret != VIDYO_CLIENT_ERROR_OK) {
            LOGI("setWindowShares...VIDYO_CLIENT_REQUEST_GET_WINDOW_SHARES returned error->%d...\n",ret);
    }
    else{
            LOGI("setWindowShares...VIDYO_CLIENT_REQUEST_GET_WINDOW_SHARES returned succ...remoteAppName->%s...numApp->%d...currApp->%d...\n",
                rws.shareList.remoteAppName,rws.shareList.numApp,rws.shareList.currApp);
    ...
    rws.requestType = CHANGE_SHARING_WINDOW;
    rws.shareList.newApp = rws.shareList.currApp + 1;

    ret = VidyoClientSendRequest(VIDYO_CLIENT_REQUEST_SET_WINDOW_SHARES, &rws, sizeof(VidyoClientRequestWindowShares));
    if (ret != VIDYO_CLIENT_ERROR_OK) {
            LOGI("setWindowShares...VIDYO_CLIENT_REQUEST_SET_WINDOW_SHARES returned error->%d...\n",ret);
    }
    else{
            LOGI("setWindowShares...VIDYO_CLIENT_REQUEST_SET_WINDOW_SHARES returned succ...\n");
    }
}
